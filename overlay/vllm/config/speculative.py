...
class SpeculativeConfig:
    ...
    def hf_config_override(hf_config: PretrainedConfig) -> PretrainedConfig:
        ...
        if hf_config.model_type == "deepseek_v4" and hasattr(
            hf_config, "dspark_block_size"
        ):
            hf_config.model_type = "dspark"
            hf_config.dflash_config = {
                "mask_token_id": hf_config.dspark_noise_token_id,
                "target_layer_ids": [
                    i - 1 for i in hf_config.dspark_target_layer_ids
                ],
                "causal": False,
                "use_swa": True,
                "use_aux_hidden_state": True,
                "block_size": hf_config.dspark_block_size,
                "markov_rank": hf_config.dspark_markov_rank,
            }
            hf_config.update(
                {
                    "n_predict": hf_config.dspark_block_size - 1,
    ...
    def requires_eagle_cache_drop(self) -> bool:
        ...
        return (
            self.use_eagle() and not self.use_dflash() and not self.use_dspark()
        )
