...
class DeepseekV4Model(nn.Module):
    ...
    def _reduce_residual_stream(
        ...
        if layer._should_run_b12x_mhc(int(hidden_states.shape[0])):
            ...
            collapsed = b12x_mhc_post(hidden_states, residual, post_mix, res_mix)
        else:
            collapsed = mhc_post_tilelang(hidden_states, residual, post_mix, res_mix)
        return collapsed.reshape(
            collapsed.shape[0], self.hc_mult, self.config.hidden_size
        ).mean(dim=1)
        ...
        for idx, layer in enumerate(
            ...
            if idx in self.aux_hidden_state_layers:
                ...
                aux_hidden_states.append(
                    self._reduce_residual_stream(
                        layer, hidden_states, residual, post_mix, res_mix
                    )
                )
